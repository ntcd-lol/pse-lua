...
--- ^         ^
--- Как это работает:
--- * Декларативная таблица всех команд SDK: имя -> { code, ["in"], ["out"] }.
--- * Поля описываются как name -> { offset, type }, где offset - нулевой
---   (совпадает с C-раскладкой), type - код sdk.pack.
--- * Раскладки перенесены из docs/ru/reference/enums_h/PseCommand.md.
--- * sdk.core использует эту схему для упаковки/распаковки без ручных
---   смещений в коде скриптов.
--- --==-==--
